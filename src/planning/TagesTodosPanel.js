...
const TagesTodosPanel = ({ items, onToggle, onAdd, onRemove }) => {
  ...
  return (
    <div className="unterricht-todos">
      <ul className="unterricht-todo-list" aria-label="Offene Todos">
        {open.length === 0 && done.length === 0 ? (
          <li className="unterricht-todo-empty">Noch keine Todos — unten hinzufügen.</li>
        ) : null}
        {open.map((item) => (
      ...
      {done.length > 0 ? (
        <ul className="unterricht-todo-list unterricht-todo-list--done" aria-label="Erledigte Todos">
          {done.map((item) => (
      ...
      <div className="unterricht-todo-add">
        <input
          ...
          aria-label="Neues Todo eingeben"
        />
        <button
          type="button"
